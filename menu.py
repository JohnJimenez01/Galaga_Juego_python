...
def show_go_screen():
	...
	if 'score' in globals():
		# Mostrar solo el puntaje final si el jugador ha jugado
		draw_text(screen, f"Puntaje final: {score}", 40, WIDTH // 2, HEIGHT // 2)
	else:
		# Pantalla de inicio con título e instrucciones
	...
	# screen.blit(background, [0,0])
	# draw_text(screen, "SHOOTER", 65, WIDTH // 2, HEIGHT // 4)
	# draw_text(screen, "No dejes que ningun meteorito te toque."
    # "muevete hacia arriba y abajo junto la tecla(SPACE) para sobrevivir", 27, WIDTH // 2, HEIGHT // 2)
	# if 'score' in globals():
	# 	draw_text(screen, f"Puntaje final: {score}", 25, WIDTH // 2, HEIGHT // 2 + 40)
